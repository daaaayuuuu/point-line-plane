...
export function ProductFactoryApp() {
  ...
  const choosePhaseStage = useCallback((stage: StageKey) => {
    setActiveStage(stage);
    const url = new URL(window.location.href);
  ...
  return (
    <div className="app-shell">
      ...
      <Workspace
        ...
        onChoosePhaseStage={choosePhaseStage}
        onOpen={openProject}
  ...
  return (
    <main className="login-layout">
      ...
      <section className="login-panel" aria-labelledby="login-title">
        <div className="login-auth-icon"><ChatGPTMark /></div>
        <header className="login-panel-header">
          <h2 id="login-title">用你的 GPT 账号开始</h2>
          <p id="login-description">通过官方授权页面登录，继续管理和交付你的产品，点线面不会保存你的 ChatGPT 密码。</p>
        </header>
        <div className="login-permissions" aria-label="登录权限说明">
          <ul>
            <li><Check size={14} aria-hidden="true" /><span>识别你的账号与登录状态</span></li>
            <li><Check size={14} aria-hidden="true" /><span>使用已授权的 GPT 能力执行产品任务</span></li>
            <li><Check size={14} aria-hidden="true" /><span>不会读取或保存你的 ChatGPT 密码</span></li>
          </ul>
        </div>
        <button
        ...
        >
          {loginState === "starting" && "正在打开官方授权…"}
          {loginState === "waiting" && "请在新窗口完成登录"}
          {loginState === "idle" && "使用 GPT 账号登录"}
        </button>
...
function Workspace({ project, projects, user, activeStage, activePhase, mobileNav, busy, onToggleNav, onChooseStage, onChoosePhase, onChoosePhaseStage, onOpen, onCreate, onDelete, onLogout, onRefresh, onProjectChange, onError, onNotice }: {
  ...
  onChoosePhaseStage: (stage: StageKey) => void;
  ...
  const currentPhaseIndex = project?.preview_accepted
  ...
  const viewingFuture = Boolean(project && viewedPhaseIndex > currentPhaseIndex);
  ...
  return (
    <main className={`workspace-layout ${mobileNav ? "sidebar-open" : ""} ${project ? "project-workbench-mode" : ""} ${isCodexWorkbench ? "codex-workbench-mode" : ""} ${project && !sidebarCollapsed ? "sidebar-expanded" : ""}`}>
      ...
      <section className="workspace-main">
        ...
        {project ? <>
          {viewingFuture && <div className="future-preview-note"><span>步骤预览</span><p>你可以先了解这里会得到什么；完成“{PRODUCT_PHASES[currentPhaseIndex].shortTitle}”后，相关操作会自动开放。</p><button type="button" onClick={() => onChoosePhase(PRODUCT_PHASES[currentPhaseIndex].key)}>返回当前步骤<ArrowRight size={15} /></button></div>}
          <StagePanel
            phase={activePhase}
            stage={activeStage}
            project={project}
            ...
            onSelectStage={onChoosePhaseStage}
          />
