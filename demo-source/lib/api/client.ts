...
async function sendCodexMessageWithReasoning(
  ...
  const poll = async () => {
    ...
    try {
      ...
      if (progress.reply) onReply?.(progress.reply);
    } catch (error) {
...
export const api = {
  ...
  demoLogin: (displayName: string) =>
    request<{ user: User }>("/auth/demo-login", {
      method: "POST",
  ...
  sendCodexMessage: (
    ...
    preferences?: { model?: string; reasoningEffort?: string; serviceTier?: "fast" | "default" },
    onReply?: (reply: string) => void,
  ) => sendCodexMessageWithReasoning(projectId, message, signal, onReasoningSummary, preferences, onReply),
  ...
  quoteProjectCost: (projectId: string, operation: "development") =>
    request<CostAuthorization>(`/projects/${projectId}/cost-quotes`, {
      method: "POST",
      body: { operation },
    }),
  confirmCostAuthorization: (authorizationId: string) =>
    request<CostAuthorization>(`/cost-authorizations/${authorizationId}/confirm`, { method: "POST" }),
  startDevelopment: (
    projectId: string,
    uiStyleKey: string,
    prototypeContract?: LowFidelityPrototypeContract,
    costAuthorizationId?: string,
  ) =>
    request<Task>(`/projects/${projectId}/development/start`, {
      method: "POST",
      body: {
        ui_style_key: uiStyleKey,
        prototype_contract: prototypeContract,
        cost_authorization_id: costAuthorizationId,
      },
    }),
  ...
  prepareProduction: (token: string, costAuthorizationId?: string) =>
    request<Task>(`/previews/${token}/prepare-production`, { method: "POST", body: { cost_authorization_id: costAuthorizationId } }),
  ...
  verifyCloudCredential: (id: string) =>
    request<CloudCredential>(`/cloud-credentials/${id}/verify`, { method: "POST" }),
  deploymentAgentMessage: (
    ...
    signal?: AbortSignal,
    preferences?: { model?: string; reasoningEffort?: string; serviceTier?: "fast" | "default" },
  ) => request<DeploymentAgentResponse>(`/projects/${projectId}/deployment-agent/messages`, {
    method: "POST",
    signal,
    body: {
  ...
  uploadDeploymentAttachment: (projectId: string, file: File) => {
    const body = new FormData();
    body.append("file", file);
    return request<DeploymentAttachment>(`/projects/${projectId}/deployment/attachments`, {
      method: "POST",
      body,
    });
  },
  selectLightweightDeployment: (projectId: string) => request<ProductionReadiness>(`/projects/${projectId}/deployment-lightweight`, {method: "POST"}),
  productionReadiness: (projectId: string) => request<ProductionReadiness>(`/projects/${projectId}/production-readiness`),
  connectDeploymentDatabase: (projectId: string, connectionUrl: string, additionalEnvironment: Record<string, string> = {}) => request<ProductionReadiness>(`/projects/${projectId}/deployment-database`, {method: "POST", body: {connection_url: connectionUrl, additional_environment: additionalEnvironment}}),
